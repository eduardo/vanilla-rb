...
describe Vanilla::Renderers::Base do
  ...
  describe "when including snips" do
    before(:each) do
      ...
      app = Vanilla::App.new(nil)
      @renderer = Vanilla::Renderers::Base.new(app)
      @snip = create_snip(:name => 'blah', :content => 'include {"some other snip"}')
    end

    it "should allow snip names to be quoted" do
      other_snip = create_snip(:name => 'some other snip', :content => 'included content')
      @renderer.render(@snip).should == "include included content"
    end
    
    it "should allow snip arguments to be quoted" do
      @snip.content = %{calling {dyna "one two three"}}
      other_snip = create_snip(:name => 'dyna', :content => 'class MyDyna; def handle(arg) arg; end; self; end', :render_as => "Ruby")
      @renderer.render(@snip).should == "calling one two three"
    end

    it "should allow multiple snip arguments to be quoted" do
      @snip.content = %{calling {dyna "one two three","four five six"}}
      other_snip = create_snip(:name => 'dyna', :content => 'class MyDyna; def handle(a, b) "#{b}/#{a}"; end; self; end', :render_as => "Ruby")
      @renderer.render(@snip).should == "calling four fix six/one two three"
    end
  end
end